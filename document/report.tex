...
\NewDocumentCommand{\tendsto}{m o m}{%
  \IfNoValueTF{#2}{%
    #1 \longrightarrow #3%
  }{%
    #1 \underset{#2}{\longrightarrow }#3}%
}
...
\begin{abstract}
We compute $\sqrt{a}$ by the Heron (Babylonian) iteration $x_{n+1}=\frac{x_n+\frac{a}{x_n}}{2}$ and
...
with $\frac{a}{x}$: if $x$ overestimates $\sqrt{a}$ then $\frac{a}{x}$ underestimates it, and their mean is
...
\begin{proof}
Multiplying through by $x$, $\dfrac{x+\frac{a}{x}}{2}=\dfrac{x^2+a}{2x}$.
\end{proof}
...
\begin{proof}
If $x>0$ then $\frac{a}{x}>0$, so $S(x)=\frac{x+\frac{a}{x}}{2}>0$. Since $x_0>0$, induction gives $x_n>0$ for
all $n$.
\end{proof}
...
For every $k\geq 0$,
\[
  x_{k+1}-\sqrt{a}\;\leq\;\Bigl(\frac{1}{2}\Bigr)^k\,(x_1-\sqrt{a}).
\]
\end{theorem}
...
\[
  x_{k+2}-\sqrt{a}\;\leq\;\frac{x_{k+1}-\sqrt{a}}{2}
   \;\leq\;\frac12\Big(\frac{1}{2}\Big)^k(x_1-\sqrt{a})
   \;=\;\Big(\frac{1}{2}\Big)^{k+1}(x_1-\sqrt{a}). \qedhere
\]
\end{proof}
...
\begin{proof}
By Lemma~\ref{lem:err}, $S(x)-\sqrt{a}=\frac{(x-\sqrt{a})^2}{2x}$. Since $0<\sqrt{a}\leq x$ we
have $2\sqrt{a}\leq 2x$, hence enlarging the denominator gives
$\frac{(x-\sqrt{a})^2}{2x}\leq\frac{(x-\sqrt{a})^2}{2\sqrt{a}}$.
\end{proof}
...
\[
  0\;\leq\;x_{n+1}-\sqrt{a}\;\leq\;\underset{\tendsto{}[n\to+\infty]{0}}{\underbrace{\Bigl(\frac{1}{2}\Bigr)^n(x_1-\sqrt{a})}},
\]
